...
import { EmaPriceChartComponent } from '../charts/ema-price-chart.component'
...
import { EMAService } from '../technical-indicators/ema.service'
import { EMA_TEST_DATA } from '../technical-indicators/ema-test-data'
...
@Component({
    ...
    providers: [YahooFinanceService, MACDService, StochasticService, EMAService],
    directives: [StockPricesChartComponent, MacdSignalChartComponent, StochasticOscillatorChartComponent, EmaPriceChartComponent]
})
export class YahooFinanceComponent implements OnInit {
    ...
    stochastic: any;
    ema: any;
    constructor(private yfs: YahooFinanceService, private macdService: MACDService, private stochasticService: StochasticService, private emaService: EMAService) {
    ...
    getHistorical() {
        this.yfs.Historical("MENT", new Date(this.startDate), new Date(this.endDate)).then(result => {
            ...
            this.ema =  this.emaService.calculate(this.historical);
        });
    }
}